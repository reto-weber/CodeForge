...
class ContainerManager:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # All methods failed
                    raise Exception(
                        f"Docker connection failed. env={env_e}, " f"socket={socket_e}"
                    )
    ...
    def create_session_container(self, session_id: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(
                f"Created container {container_name} " f"for session {session_id}"
            )
            return container.id

        except Exception as e:
            logger.error(
                f"Failed to create container for " f"session {session_id}: {e}"
            )
            return None
        ...
        try:
            ...
            try:
                ...
                if container.status != "running":
                    logger.info(
                        f"Container {container.name} is not running, " f"recreating..."
                    )
                    if not self.create_session_container(session_id):
                        return False, "Failed to recreate container", -1
            ...
            if language == "python":
                ...
                if match:
                    ...
                    cmd = f"javac {filename} && " f"java $(basename {filename} .java)"
            else:
            ...
            logger.info(
                f"Code execution completed for session {session_id}: "
    ...
    def cancel_execution(self, session_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Error cancelling execution for " f"session {session_id}: {e}"
                )
                return False
        ...
        try:
            if session_id in self.active_containers:
                ...
                try:
                    ...
                    container.remove()
                    logger.info(f"Cleaned up container for " f"session {session_id}")
                except Exception as e:
                    logger.warning(
                        f"Error stopping/removing container for "
                ...
                try:
                    ...
                    container.stop(timeout=5)
                    container.remove()
                    logger.info(f"Cleaned up orphaned container " f"{container_name}")
                except docker.errors.NotFound:
                    pass  # Container doesn't exist, which is fine
                except Exception as e:
                    logger.warning(
                        f"Error cleaning up orphaned container "
                        f"{container_name}: {e}"
                    )
            ...
            logger.error(
                f"Error cleaning up container for " f"session {session_id}: {e}"
            )
            return False
